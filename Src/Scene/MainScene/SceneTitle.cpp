...
SceneTitle::SceneTitle(void)
	...
	, peachCollider_(nullptr)
	, waveCollider_(nullptr)
	, oniSimaCollider_(nullptr)
	, isSelectMenu_(true)
{
...
void SceneTitle::InitUI(void)
{
	...
	// ソロプレイボタン
	const Vector2F POS_SOLO(CENTER_X, START_Y + (INTERVAL_Y * 0.0f));
	soloPlayButtonCollider_ = std::make_unique<Collider2DBox>
		(POS_SOLO, BUTTON_WIDTH, BUTTON_HEIGHT, Collider2DBase::TAG_2D::SOLO_PLAY_BUTTON);
	...
	// マルチプレイボタン
	const Vector2F POS_MULTI(CENTER_X, START_Y + (INTERVAL_Y * 1.0f));
	multiPlayButtonCollider_ = std::make_unique<Collider2DBox>(POS_MULTI, BUTTON_WIDTH,
	...
	// 設定ボタン
	const Vector2F POS_OPTION(CENTER_X, START_Y + (INTERVAL_Y * 2.0f));
	optionButtonCollider_ = std::make_unique<Collider2DBox>(POS_OPTION, BUTTON_WIDTH,
	...
	// 終了ボタン
	const Vector2F POS_EXIT(CENTER_X, START_Y + (INTERVAL_Y * 3.0f));
	exitButtonCollider_ = std::make_unique<Collider2DBox>(POS_EXIT, BUTTON_WIDTH,
		BUTTON_HEIGHT, Collider2DBase::TAG_2D::EXIT_BUTTON);
	CollisionController::GetInstance().RegisterCollider2D(exitButtonCollider_.get());
	CollisionController::GetInstance().SetCollisionGroup2D(Collider2DBase::TAG_2D::MOUSE_CURSOR,
		Collider2DBase::TAG_2D::EXIT_BUTTON, true);
	...
	buttonTags =
	{
		...
		Collider2DBase::TAG_2D::EXIT_BUTTON
	};

#ifdef _DEBUG
	// 座標の初期化
	const Vector2F PEACH_POS(100.0f, Application::SCREEN_HALF_Y + 100);
	const Vector2F WAVE_POS(800.0f, Application::SCREEN_HALF_Y + 250);
	const Vector2F ONI_SIMA_POS(Application::SCREEN_SIZE_X - 100.0f, Application::SCREEN_HALF_Y - 50);

	// 桃の当たり判定
	peachCollider_ = std::make_unique<Collider2DBox>(PEACH_POS, 300.0f, 300.0f,
		Collider2DBase::TAG_2D::PEACH);

	// 波の当たり判定
	waveCollider_ = std::make_unique<Collider2DBox>(WAVE_POS, Application::SCREEN_SIZE_X,
		Application::SCREEN_HALF_Y, Collider2DBase::TAG_2D::WAVE);

	// 鬼ヶ島の当たり判定
	oniSimaCollider_ = std::make_unique<Collider2DBox>(ONI_SIMA_POS, 200.0f,
...
void SceneTitle::Update(void)
{
	...
	// マウス座標の更新
	const Vector2 MOUSE_POS = keyConfInputManager.GetMousePosition();
	const Vector2F MOUSE_POS_F(static_cast<float>(MOUSE_POS.x), static_cast<float>(MOUSE_POS.y));
	cursorCollider_->SetCenterPos(MOUSE_POS_F);
	...
	if (isSelectMenu_)
	{
		...
		for (int i = 0; i < MENU_BUTTON_NUM; ++i)
		{
			const bool IS_TARGET = CollisionController::GetInstance().IsTagCollidingWithTag2D(Collider2DBase::TAG_2D::MOUSE_CURSOR, buttonTags[i]) || (selectedIdx_ == i);

			if (IS_TARGET && keyConfInputManager.isTrigerDown("OK"))
			{
				isSelectMenu_ = false;
...
void SceneTitle::Draw(void)
{
	...
	const float BOB_SPEED = 3.0f;
	const float OFFSET_Y = std::sinf(time_ * BOB_SPEED) * AMPLITUDE;
	...
	shaderCtrl.Draw2D(
		ResourceManager::SRC::PS_NORMAL_MAP,
		-20, static_cast<int>(Application::SCREEN_HALF_Y + OFFSET_Y),
		PEACH_SCALE,
		peachParams_,
		peachHandle_,
		peachNormalHandle_
	...
	for (int i = 0; i < MENU_BUTTON_NUM; ++i)
	{
		const bool IS_SELECTED = CollisionController::GetInstance().
			IsTagCollidingWithTag2D(TAG_2D::MOUSE_CURSOR, buttonTags[i])
			|| (selectedIdx_ == i);

		const int IMG_IDX = IS_SELECTED ? (i * 2 + 1) : (i * 2);

		const Vector2F POS = colliders[i]->GetCenterPos();
		DrawRotaGraph(static_cast<int>(POS.x), static_cast<int>(POS.y), DEFAULT_SCALE,
			UtilityMath::DEG2RAD, imageMenu_[IMG_IDX], true);
	}
